...
2. Acid-base runs on three clocks and only two of them turn inside one simulation run: blood buffering (minutes; strong acid titrates HCO3- roughly 1:1), ventilatory compensation (minutes, via chemoreceptor to ventilation to PaCO2), and renal acid excretion with HCO3- regeneration (3-5 days). Respiratory acidosis compensation: acute, HCO3- rises about 1 mmol/L per 10 mmHg rise in PaCO2; chronic, 3-5 mmol/L per 10 mmHg, with full renal compensation needing 3-4 days. Which of those clocks this run actually spanned is a fact you must read from session_summary.simulated_duration_seconds, never assume. Under about six simulated hours the renal limb cannot have moved, so the ABSENCE of renal compensation is expected model behavior, not a learner error and not a model defect, and the chronic expectation must not be applied. Over about one simulated day the renal limb IS in play and the chronic figures above are the right yardstick: this simulator has a days lens and a three-day observation window, and a run of five to seven simulated days will show HCO3- moving by several mmol/L. In that case say so, and check the observed ratio against the 3-5 mmol/L per 10 mmHg band rather than declaring the compensation impossible.
...
HOW THE LEARNER WATCHED IS EVIDENCE, AND IT BELONGS TO SECTION 2. time_scale describes the observation settings, never a physiological fact, and a compression setting is never a clinical intervention. If time_scale is null or empty, skip it silently. Otherwise judge three things, each with its own remedy. First, legibility: illegible_observation_seconds above zero, or a very high peak_compression, means the trajectory shown to the learner was moving faster than it could be read, so conclusions drawn during that stretch rest on a picture nobody could see - say this BEFORE criticizing decisions taken in it. Second, patience: compare each mechanism's clock against session_summary.simulated_duration_seconds before judging it. A slow-layer parameter that barely moved inside a run of simulated minutes is expected model behavior, not a failed intervention and not a model defect; name the layer when you invoke this. The reverse is equally binding - in a run spanning days the hours-and-days limbs have had time to act, and a slow parameter that did NOT move there is a finding rather than an excuse. Third, control: automatic_hold_back_events above zero means automatic scaling protected the learner by refusing to accelerate a deteriorating patient, while many manual_lens_switches with automatic_scaling_used false means that protection was never available. When failed_within_ten_seconds_under_manual_scale is true, state plainly that the run ended within ten real seconds and that the first thing to correct is the observation scale, not the physiology.
...
（2）酸碱必须区分三个时间尺度：血液缓冲（分钟）、肺代偿（分钟）、肾脏排酸与再生 HCO₃⁻（3～5 天）。本轮究竟跨越了哪几个尺度，必须读 session_summary.simulated_duration_seconds，不得默认。模拟时长不足约 6 小时时，肾脏代偿来不及发生，“看不到肾脏代偿”是模型的正确行为，不得判为学习者失误或模型缺陷，也不得套用慢性代偿预期值；模拟时长超过约 1 天时，肾脏代偿已经在跑（本模拟器有天级镜头与 3 天观察窗，5～7 个模拟日可见 HCO₃⁻ 变动数 mmol/L），此时应当明确指出它，并用“每 10 mmHg PaCO₂ 对应 3～5 mmol/L HCO₃⁻”这一慢性代偿区间去核对实测值，而不是声称它不可能出现。
...
async function callMedicalAnalysis(report, options={}){
  ...
  const text=normalizeMedicalMarkdown(choice?.message?.content).slice(0,MAX_OUTPUT_CHARS);
  if(!text){
    // 思考型模型可能把整个 max_tokens 预算花光在 reasoning_content 上，一个可见字符都没留下：
    // HTTP 200、finish_reason='length'、content=''。笼统报"空响应"会把人引去查 key、查网络、
    // 查 prompt，而真正该动的是 HOMEOSTASIS_AI_THINKING 或 HOMEOSTASIS_AI_MAX_TOKENS。
    // 2026-09-05 公网 deepseek-v4-flash-0731 正是这样：8000 tokens 全进 reasoning，正文为空。
    if(choice?.finish_reason==='length' && choice?.message?.reasoning_content){
      throw new Error('ai_reasoning_used_whole_budget');
    }
